...
// Function to update job sites based on country
function updateJobSites(countryCode) {
    // Clear current job sites
    jobSitesGrid.innerHTML = '';

    // Get job sites for the selected country
    ...
    locationStatus.innerHTML = `
                ...
                <span>Showing job sites for: <span class="detected-location">${countryNames[countryCode]}</span></span>
            `;
...
function detectLocation() {
    ...
    // For demo purposes, we'll simulate detection with a random country
    const countries = Object.keys(jobSitesByCountry);
    const randomCountry = countries[Math.floor(Math.random() * countries.length)];

    // Simulate API delay
    setTimeout(() => {
        updateJobSites(randomCountry);
